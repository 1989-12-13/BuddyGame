...
一款面向公众的公益急救调度叙事游戏。玩家值班时接听一通通来电，完成地点确认、情况问询、路线规划、派车、电话指导与现场交接。

## 玩法

- 并发值班：三条线路、两辆车同时运转；一次只处理一条线，其余线路会响铃、会超时
- 班次没有固定通数，何时收班由表现驱动的热度模型决定（撑过峰值段圆满交班，连续漏接或患者死亡会被换下来）
- 场景练习可单独重玩 33 类主诉中的任意一个
