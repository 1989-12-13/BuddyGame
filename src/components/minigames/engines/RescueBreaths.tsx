...
export function RescueBreaths({ spec, paused, onComplete }: MiniGameProps) {
  ...
  return <section className="breath-game" aria-label="人工呼吸节奏练习">
    ...
    <svg viewBox="0 0 240 140" role="img" aria-label="胸廓起伏示意"><path d="M70 20Q120 0 170 20L190 130H50Z" fill="var(--bg-raised)" stroke="var(--text-2)"/><ellipse cx="97" cy="76" rx={22 + Math.min(elapsed, 1) * (holding ? 7 : 0)} ry="36" fill="var(--accent)"/><ellipse cx="143" cy="76" rx={22 + Math.min(elapsed, 1) * (holding ? 7 : 0)} ry="36" fill="var(--accent)"/><path d="M120 18V65M120 45L98 67M120 45L142 67" fill="none" stroke="var(--bg)" strokeWidth="5"/></svg>
