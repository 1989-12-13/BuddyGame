...
export function handleAskQuestion(state: WorldState, questionId: string, turn?: TurnModifiers): WorldState {
  ...
  if (scripted) {
    ...
    if (canFill && scripted.fillTerminal) {
      const fill = scripted.fillTerminal
      const patch: Partial<typeof newTerminal> = { ...fill } as Partial<typeof newTerminal>
      // 脚本里协议号可能写成字符串，终端字段只接受数字
      if (fill.protocolNumber != null) patch.protocolNumber = Number(fill.protocolNumber)
      newTerminal = { ...newTerminal, ...patch }
    }
    ...
    // 语气 → 态度证据：与非脚本版保持一致。
    // 脚本分支原本漏了这笔账，导致「接线态度」维度在有脚本的场景里永远为 0。
    const sDelta = turn?.stressDelta ?? 0
    const sAttitudeEvidence = {
      ...state.attitudeEvidence,
      supportiveTurns: state.attitudeEvidence.supportiveTurns + (sDelta < 0 ? 1 : 0),
      neutralTurns: state.attitudeEvidence.neutralTurns + (sDelta === 0 ? 1 : 0),
      pressuringTurns: state.attitudeEvidence.pressuringTurns + (sDelta > 0 ? 1 : 0),
      playerCausedLossControl: state.attitudeEvidence.playerCausedLossControl
        || (sDelta > 0 && cs.stressLevel !== '失控' && sStressLevel === '失控'),
    }
    ...
    return {
      ...
      attitudeEvidence: sAttitudeEvidence,
