import { Activity, ArrowRight, BookOpen, Headphones, HeartHandshake, Map, Moon, Play, RotateCcw, Sun } from 'lucide-react'
...
export function TitleScreen({ onStart, onLevelSelect, onKnowledge }: Props) {
  ...
  return <main className="dispatch-title">
    ...
    <div className="title-layout"><section className="title-copy"><span className="eyebrow">ON THE OTHER END OF THE LINE</span><h1>电话这头，<br />是<span>希望</span>的起点。</h1><p className="title-intro">一座城市，五通来电。<br />戴上耳机，听清那些慌乱的声音。<br />你的每一次确认，都让帮助更近一点。</p>
      <div className="title-actions"><button className="primary" onClick={() => onStart()}><Headphones size={20} /> 开始值班<ArrowRight size={20} /></button>{saved && <button className="secondary" onClick={() => onStart('__resume__')}><RotateCcw size={18} /> 继续第 {saved.callIndex + 1} 通</button>}</div>
      <div className="title-secondary"><button onClick={onLevelSelect}><Map size={17} /> 场景练习</button><button onClick={onKnowledge}><BookOpen size={17} /> 急救知识</button><button onClick={() => onStart('__random__')}><Play size={16} /> 随机班次</button></div>
      <div className="title-caption"><span className="caption-rule" /><p>第一次独立值班<br /><span>从倾听开始，直到有人接过你的手。</span></p></div>
      ...
      <div className="art-note"><HeartHandshake size={17} /> 让快乐和善意，同时发生</div>
    </section></div>
    <footer className="title-footer"><span>倾听 · 确认 · 行动</span><p>公益科普体验，不能替代专业急救培训。现实紧急情况请拨打 120。</p><span>DEMO / 01</span></footer>
  </main>
}
