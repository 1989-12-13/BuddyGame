...
export function QuestionDock({ state, dispatch }: { state: WorldState; dispatch: Dispatch<GameAction> }) {
  ...
  const canAsk = (id: string) => {
    const attempts = state.callerState!.questionAttempts[id] ?? 0
    if (attempts === 0) return true
    return attempts < 2
      && state.callerState!.questionQuality[id] !== 'clear'
      && state.callerState!.stress < (state.callerState!.questionStress[id] ?? state.callerState!.stress)
  }
  ...
  return <section className="question-dock"><header><MessageCircle size={17} /><h3>下一句，问什么？</h3><span>{busy ? `问询中 · ${state.actionEndsAt - state.shiftElapsed} 秒` : '依据已经听到的信息，选择重点'}</span></header>
    ...
    {state.currentCall.mpdsQuestions.length > 0 && <details><summary>补充问询</summary><div className="question-buttons">{state.currentCall.mpdsQuestions.map(q => <button key={q.id} disabled={busy || !canAsk(q.id) || !!q.prerequisites?.some(id => !asked.includes(id))} onClick={() => dispatch({ type: 'ASK_QUESTION', questionId: q.id })}>{(state.callerState!.questionAttempts[q.id] ?? 0) > 0 && canAsk(q.id) && <RotateCcw size={14} />}{canAsk(q.id) && (state.callerState!.questionAttempts[q.id] ?? 0) > 0 ? `再确认${q.label}` : q.label}</button>)}</div></details>}
